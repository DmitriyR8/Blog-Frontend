<header>
    ...
    <div class = 'header-bottom'>
        <div class = "about-site" style = "background: linear-gradient(rgba(51,51,51,0.5), rgba(51,51,51,0.5)), url({{env('API_URL').'show-article-image/'.$getArticleById['slug']}}) no-repeat; background-size: 100% 100%;">
            <div class = "about-site-header">
                <h1>
                    {{$getArticleById['h1_title']}}
                </h1>
            </div>
            <div class = 'about-site-bottom'>
                <div class = 'about-site-bottom-header'>
                    <p>{{$getArticleById['short_desc']}}</p>
                </div>
                <div class = 'view-info'>
                    <span>{{Carbon\Carbon::parse($getArticleById['created_at'])->format('Y-m-d')}}</span>
                    <span>|</span>
                    <span>{{$getArticleById['author']}}</span>
                    <span>|</span>
                    <span>Read {{$getArticleById['read_time']}} minutes</span>
                </div>
            </div>
        </div>
        <div class = 'about-site-bottom-mob'>
                <div class = 'about-site-bottom-header'>
                    <p>{{$getArticleById['short_desc']}}</p>
                </div>
                <div class = 'view-info'>
